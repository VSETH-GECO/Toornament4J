...
public class Toornament4J {
    ...
    static {
        InputStream stream = Toornament4J.class.getClassLoader().getResourceAsStream("lib.properties");
        Properties properties = new Properties();
        try {
            properties.load(stream);
            stream.close();
        } catch (IOException e) {
            Toornament4J.LOGGER.error(LogMarkers.MAIN, "Toornament4J Internal Exception", e);
        }
    ...
    /**
     * Initializes the logger as either the {@link Toornament4JLogger default implementation} or a found SLF4J implementation.
     *
     * @return The initialized logger.
     */
    private static Logger initLogger() {
        if (!isSLF4JImplementationPresent()) {
            System.err.println("Toornament4J: ERROR INITIALIZING LOGGER!");
            System.err.println("Toornament4J: No SLF4J implementation found, reverting to the internal implementation ("+Toornament4JLogger.class.getName()+")");
            System.err.println("Toornament4J: It is *highly* recommended to use a fully featured implementation like logback!");
            return new Toornament4JLogger(Toornament4J.class.getName());
        } else {
            return LoggerFactory.getLogger(Toornament4J.class);
        }
    }
    ...
    /**
     * A logger implementation used by Toornament4J if no valid SLF4J implementation is found.
     */
    public static class Toornament4JLogger extends MarkerIgnoringBase {

        private final String name;
        private volatile int level = Level.INFO.ordinal();
        private volatile PrintStream standard, error;

        public Toornament4JLogger(String name) {
            this.name = name;
            standard = System.out;
            error = System.err;
        }
